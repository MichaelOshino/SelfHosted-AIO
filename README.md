...
- [Mailu](https://github.com/Mailu/Mailu) - почтовый сервер. 
...
&emsp; На полный запуск может потребоваться несколько минут. 
...
<details>
  <summary>Mailu</summary>

&emsp; Должен запускаться на отдельном сервере, который будет выполнять только роль почтового сервера.

&emsp; Для корректной работы почты необходимо указать почтовые DNS для домена (MX, SPF, DKIM, DMARC) со стороны держателя NS домена.
Сами DNS можно увидеть авторизовавшись под администратором в админ-панели: **Почтовые домены** -> **Действия** -> **Подробнее** (первая кнопка) -> **Сгенерировать ключи**  
**Не забудьте создать поддомен, указанный в MX, и привязать к серверу.**  
&emsp; Так же требуется указать rDNS (PTR) запись для IPv4 адреса, обычно это можно сделать в панели хостера у которого был заказан сервер. 
В rDNS (PTR) должен быть указан первый поддомен из списка в hostnames (см. предварительную настройку при запуске).
</details>
